...
const Estagiarios = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const apiUrl = `http://localhost:3001/api/estagiarios?${queryParams.toString()}`;
  ...
  const handleViewDetails = (estagiario: any) => {
    navigate(`/detalhes?tipo=estagiario&id=${encodeURIComponent(estagiario.id || estagiario.name)}&nome=${encodeURIComponent(estagiario.name)}`);
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            ...
            >
              ...
              >
                ...
                >
                  <motion.button 
                    type="submit" 
                    className="w-full bg-[#F97316] hover:bg-[#E65E05] text-white px-4 py-2 rounded disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            {hasSearched && results.length > 0 && (
              <>
                ...
                >
                  {currentResults.map((estagiario, index) => (
                    <motion.div
                      key={estagiario.id}
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        {estagiario.conselho && estagiario.conselho !== "N/D" && (
                          <div>
                            <p className="font-medium text-foreground">Conselho Regional</p>
                            <p className="text-foreground">{estagiario.conselho}</p>
                          </div>
                        )}
                        {estagiario.cedula && estagiario.cedula !== "N/D" && (
                          <div>
                            <p className="font-medium text-foreground">Cédula</p>
                            <p className="text-foreground">{estagiario.cedula}</p>
                          </div>
                        )}
                        {estagiario.morada && estagiario.morada !== "N/D" && (
                          <div>
                            <p className="font-medium text-foreground">Morada</p>
                            <p className="text-foreground">{estagiario.morada}</p>
                          </div>
                        )}
                        
                        {estagiario.localidade && estagiario.localidade !== "N/D" && (
                          <div>
                            <p className="font-medium text-foreground">Localidade</p>
                            <p className="text-foreground">{estagiario.localidade}</p>
                          </div>
                        )}
                        {estagiario.codigo_postal && estagiario.codigo_postal !== "N/D" && (
                          <div>
                            <p className="font-medium text-foreground">Código Postal</p>
                            <p className="text-foreground">{estagiario.codigo_postal}</p>
                          </div>
                        )}
                        
                        {estagiario.email && estagiario.email !== "N/D" && (
                          <div>
                            <p className="font-medium text-foreground">Email</p>
                            <p className="text-foreground">{estagiario.email}</p>
                          </div>
                        )}
                        
                        {estagiario.telefone && estagiario.telefone !== "N/D" && (
                          <div>
                            <p className="font-medium text-foreground">Telefone</p>
                            <p className="text-foreground">{estagiario.telefone}</p>
                          </div>
                        )}
                        
                        {estagiario.fax && estagiario.fax !== "N/D" && (
                          <div>
                            <p className="font-medium text-foreground">Fax</p>
                            <p className="text-foreground">{estagiario.fax}</p>
                          </div>
                        )}
                      </div>
                    </motion.div>
          ...
          {/* Right column - Featured trainees */}
          <div className="lg:col-span-1">
            <motion.div 
              ...
              whileHover={{ boxShadow: "0 10px 25px -5px rgba(0, 0, 0, 0.1)" }}
            >
              <h2 className="text-lg font-medium text-foreground mb-4">Estagiários em Destaque</h2>
              <p className="text-foreground text-sm mb-4">Descubra estagiários de advocacia e as suas informações comprovadas.</p>
              <div className="space-y-4">
                {[1, 2, 3].map((_, index) => (
                  <motion.div 
                    key={index}
                    className="flex items-start space-x-4 bg-background p-4 rounded-lg shadow-sm border border-border"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.3, delay: index * 0.1 }}
                    whileHover={{ boxShadow: "0 4px 12px rgba(0, 0, 0, 0.1)" }}
                  >
                    ...
                    <div>
                      <h3 className="font-semibold text-foreground">Estagiário de Exemplo {index + 1}</h3>
